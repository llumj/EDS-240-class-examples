...
ggplot(data = penguins,
  ...
  geom_point() + 
  theme_minimal() + 
  scale_color_manual(values = c("cornflowerblue", "red4", "orange3"))
```
...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm, y = bill_depth_mm,
                     colour = body_mass_g)) + 
geom_point() +
scale_color_gradient(low = "cornflowerblue", high ="yellow")
```
...
geom_point(data= penguins_summary,
  mapping = aes(x = mean_bill_length_mm, y = mean_bill_depth_mm, fill = species), size = 6, shape = 24)
```
